...
# ============================================================================
# BENCHMARK VARIANT: ELLA Adam.
# This file is a copy of the shipped (Newton) ELLA/ELLA.py with ONLY the
# alternative-model fit reverted to the original ELLA-v1 per-kernel Adam optimizer
# (restored from git d96eb76). Registration (ray-cast), nhpp_prepare, compute_pv
# (scipy FDR), and weighted_density_est are byte-identical to the shipped Newton
# version, so a Newton-vs-Adam comparison isolates the optimizer alone. The Newton
# solver below is left in place (unused) to keep the diff minimal.
# Used only by competing_methods/ELLA1/benchmark/; NOT part of the shipped package.
# ============================================================================
...
# ------------------------------------------------------------------ Adam fit
# Original ELLA-v1 alternative-model machinery (restored from git d96eb76). The
# per-location log-likelihood of lambda = exp(B)*r + exp(A)*kernel is optimized
# over (A, B) = (logA, logB) with Adam + adaptive-LR early stopping, once per beta
# kernel. This is the arm compared against the bounded-Newton fit.
class model_beta(torch.nn.Module):
    '''NHPP torch model: location-wise log-likelihood of the alternative model
    with a single beta kernel, parameterized by A=logA, B=logB (so exp keeps A,B>=0).'''
    ...
    def forward(self, ri, c0i, ni, a, b, ri_clamp_min, ri_clamp_max, L1_lam):
        ...
        lli = torch.log(c0i*2.0*math.pi) + torch.log((torch.exp(self.A)/BB)*torch.pow(ri_clamp,a)*torch.pow(1-ri_clamp,b-1) + torch.exp(self.B)*ri_clamp) - (c0i*2.0*math.pi/ni)*((torch.exp(self.A)*a)/(a+b)+(torch.exp(self.B))/(2.0)) - L1_lam*torch.exp(self.A)
        return lli


def loss_ll(pred):
    '''Negative summed log-likelihood, for minimization.'''
...
class ELLA:
    ...
    Class of ELLA
    '''
    def __init__(self, dataset='untitled', beta_kernel_param_list=None, adam_learning_rate_max=1e-2, adam_learning_rate_min=1e-3, adam_learning_rate_adjust=1e7, adam_delta_loss_max=1e-2, adam_delta_loss_min=1e-5, adam_delta_loss_adjust=1e8, adam_niter_loss_unchange=20, max_iter=5000, min_iter=100, newton_max_iter=100, newton_ftol=1e-12, max_ntanbin=25, n_angles=360, ri_clamp_min=0.0, ri_clamp_max=1.0-1e-5, lam_filter=0.0, L1_lam=0):
        '''
        Constructor
        Args:
        ...
        # NHPP fit constants (Adam alternative fit -- benchmark variant)
        self.newton_max_iter = newton_max_iter   # unused (Newton solver left in place)
        self.newton_ftol = newton_ftol           # unused
        ...
        for it, t in enumerate(self.type_list):
            ...
            for _ig, g in enumerate(tqdm(_gl, desc=f'NHPP fitting for {t}')):
                ...
                if (len(r_g)>0):
                    ...
                    ## NHPP (alternative) fit: original ELLA-v1 per-kernel Adam over
                    ## (logA, logB) of lambda = exp(B)*r + exp(A)*kernel, with adaptive LR
                    ## and delta-loss early stopping. Stores A_est/B_est in LOG scale, which
                    ## weighted_density_est() exps -- identical convention to the Newton arm.
                    init_A = -10
                    init_B_alter = np.log(np.maximum(hatB, 1e-7))
                    r = torch.reshape(torch.Tensor(np.array(r_g)), (len(r_g), 1))
                    c0 = torch.reshape(torch.Tensor(np.array(c0_g)), (len(r_g), 1))
                    n = torch.reshape(torch.Tensor(np.array(n_g)), (len(r_g), 1))
                    # adaptive learning rate + delta-loss from the homogeneous loglik
                    ...
                    for k in range(self.nk):
                        ...
                        for step in range(0, self.max_iter):
                            ...
                            # early stopping on stalled loss
                            ...
                            loss.backward()
                            optimizer.step()
                        # recompute the loss at the final params so mll matches them
                        with torch.no_grad():
                            final_loss = loss_ll(model(r, c0, n, aa, bb, self.ri_clamp_min, self.ri_clamp_max, self.L1_lam))
                        A_est_t[ig, k] = float(model.A.data)
                        B_est_t[ig, k] = float(model.B.data)
                        mll_est_t[ig, k] = float(- final_loss)
