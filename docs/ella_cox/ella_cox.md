---
title: ELLA-Cox
layout: default
nav_order: 8
has_children: true
has_toc: false
---
## ELLA-Cox (archived)

Tutorial for the ELLA-Cox variant, based on a Cox-process model. Kept here, frozen,
for reference; its source lives on the repository's `ella-cox` branch.

- [Home]({{ site.baseurl }}/ella_cox/home.html)
- [Install ELLA-Cox]({{ site.baseurl }}/ella_cox/install.html)
- [A Minimum Demo]({{ site.baseurl }}/ella_cox/mini_demo.html)
- [A Complete Demo]({{ site.baseurl }}/ella_cox/complete_demo.html)
- [ELLA's Inputs]({{ site.baseurl }}/ella_cox/inputs.html)
- [Reproducibility]({{ site.baseurl }}/ella_cox/reproduce/)
